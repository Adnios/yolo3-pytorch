    ...
    for iteration in range(epoch_size):
        ...
        with torch.no_grad():
            images = Variable(torch.from_numpy(
                images).cuda().type(torch.FloatTensor))
    ...
    for iteration in range(epoch_size_val):
        ...
        with torch.no_grad():
            images_val = Variable(torch.from_numpy(
                images_val).cuda().type(torch.FloatTensor))
...
if __name__ == "__main__":
    ...
    # 使用预训练权重文件，可以加速训练中loss的收敛。也可以不适用，那样无非需要训练的时间更长些。
    ...
    if True:
        ...
        for epoch in range(Freeze_Epoch, Unfreeze_Epoch):
            fit_ont_epoch(net, yolo_losses, epoch, epoch_size,
                          epoch_size_val, gen, gen_val, Unfreeze_Epoch)
            lr_scheduler.step()
